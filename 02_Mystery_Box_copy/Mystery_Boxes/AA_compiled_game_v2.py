...
class Export:
    ...
    def save_history(self, partner, game_history, game_stats):
        ...
        # Prepare statements for writing to file
        start_balance = game_stats[0]
        end_balance = game_stats[1]
        if start_balance > end_balance:
            won_lost = "You lost ${:.2f}".format((end_balance - start_balance)*-1)
        else:
            won_lost = "You won ${:.2f}".format(start_balance - end_balance)

        start_bal_str = "Starting Balance: ${:.2f}".format(start_balance)
        end_bal_str = "Final Balance: ${:.2f}".format(end_balance)

        game_stats_list = [start_bal_str, end_bal_str, won_lost]
